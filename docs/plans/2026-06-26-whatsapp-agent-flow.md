...
- [x] **T-Cron — `GET /api/whatsapp/cron`** (`src/app/api/whatsapp/cron/route.ts`): portable, platform-agnostic job driver. Drains `generation_jobs` via `runDueJobs` (lease-bounded, returns within the serverless cap). Secured by a single pre-shared `CRON_SECRET`, accepted as `Authorization: Bearer` (Vercel Cron), `X-Cron-Secret`, or `?key=` (header-less pingers); constant-time, fails closed. Coexists with the T4 Netlify Scheduled Function (lease-claim makes both safe; enable ONE in prod). tsc+eslint+`next build` clean. *(E1 — the engine starter)*

**— MVP BUILD PHASE COMPLETE (founder directive 2026-06-26). The end-to-end loop ships: WhatsApp note → Kapso webhook → durable queue → cron driver → OpenAI worker (Whisper + gpt-4o, Turn-Budget-1) → draft + review token → mobile `/review` approve → publish → headless WP. Remaining tasks below are explicitly DEFERRED (post-MVP), NOT in scope. —**

- [ ] **T6 — Localization** *(DEFERRED — MVP ships Catalan-only per founder directive)*: route agent + `/review` copy through `tr(locale,key)`; detect inbound language.
- [ ] **T7 — Onboarding** *(DEFERRED)*: dashboard "El teu agent de WhatsApp" card + bind/verify flow + first-note replay. *(DX1,DX2)*
- [ ] **T8 — `/admin/agent` observability** *(DEFERRED)*: jobs board, per-thread timeline + transcript, cost meter. *(DX5,DX9)*
- [ ] **T9 — Outcome-loop capture** *(DEFERRED)*: 60-day rank/traffic check on the `wa_article_outcomes` records already written by T4/T5. *(G3 moat-as-data)*
- [ ] **T10 — Operator DX** *(DEFERRED)*: Kapso/`CRON_SECRET` vars in `.env.example`, provisioning + webhook-secret runbook, `scripts/wa-replay.mjs` signed replay harness. *(DX6,DX7,DX8)*
- [ ] **T11 — Tests** *(DEFERRED)*: the 12 Eng codepaths (webhook verify, job lifecycle+re-driver, idempotency, token mint/consume/expire/replay, transcription fail, gen timeout, publish, SSRF rejection, cost rate-limit). Keep `npm run test:render` green.

**Build order (shipped):** T1 → T2 → T3 → T4 → T5 → T-Cron ✅. Deferred (post-MVP): T6–T11.
...
- **STATUS: APPROVED (2026-06-26) → MVP BUILT (2026-06-26).** Core loop shipped: T1→T2→T3→T4→T5→T-Cron. T6–T11 deferred post-MVP by founder directive (Catalan-only, skip onboarding/observability/tests for launch).
