...
export type SlideButtonProps = ButtonProps & {
  /** Fires once the user drags past the threshold (e.g. navigate / unlock). */
  onComplete?: () => void
  /** Optional hint shown centred in the track. */
  label?: string
}

// Carma gold edition: the fill + handle are gold (bg-accent / primary button), the
// track is the warm surface. `shadow-button*` placeholders are swapped for real
// Tailwind shadows. Drop in for "slide to confirm/unlock" actions.
const SlideButton = forwardRef<HTMLButtonElement, SlideButtonProps>(
  ({ className, onComplete, label, ...props }, ref) => {
    ...
    return (
      <motion.div
        ...
        className="relative flex h-9 items-center justify-center rounded-full bg-surface-subtle border border-border shadow-inner"
      >
        {!completed && (
          <motion.div
            style={{
        ...
        {!completed && label && (
          <span className="pointer-events-none absolute inset-0 z-[1] flex items-center justify-center pl-6 text-xs font-bold text-on-accent/80">
            {label}
          </span>
        )}
          ...
          {!completed && (
            ...
            >
              <Button
                ...
                size="icon"
                className={cn(
                  "rounded-full shadow-md drop-shadow-xl",
