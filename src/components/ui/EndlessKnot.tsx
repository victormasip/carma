 ...
 *
 * IT MUST BE `0 0 50 50`, AND THE ROTATION IS WHY.
 *
 * The weave's own bounding box is x 5.980…44.021, y 0.771…49.229 — dead centre
 * on (25, 25) in the source art's coordinates. The box used to read
 * `-2.604 0 50 50`, which put the viewBox centre at (22.396, 25) and therefore
 * drew the mark 2.604 user units — 5.2% of its own size — to the RIGHT of the
 * element it lives in.
 *
 * That offset is invisible while the mark is still and unmissable the moment it
 * turns: every rotate() on a wrapper spins around the ELEMENT's centre, so the
 * knot orbited a circle of radius 5.2% instead of turning on the spot — 45px at
 * the landing's 860px hero mark. Founder, 2026-09-18: the knot "acts weird /
 * glitchy when rotating". It was not the animation; it was the frame.
 *
 * Every consumer (this file, KnotLoader, KnotSpinner, the landing's closing tie)
 * shares this constant, so they all move together and stay registered.
 */
export const KNOT_VIEWBOX = '0 0 50 50'
